package com.o2.edh.processors.mddif.storage.local;
...
//import static com.o2.edh.mddif.csv.StandardValidators.POSITIVE_INTEGER_VALIDATOR;
//import static com.o2.edh.mddif.csv.StandardValidators.TIME_PERIOD_VALIDATOR;
import org.apache.nifi.processor.util.StandardValidators;
/**
 * Created by aditya.thakare on 10/10/2019.
 */

@TriggerSerially
@Tags({"MDDIF","file", "get", "list", "ingest", "source", "filesystem"})
...
@WritesAttributes({
        ...
        @WritesAttribute(attribute= MDDIFListFile.FILE_OWNER_ATTRIBUTE, description="The user that owns the file in filesystem"),
        @WritesAttribute(attribute= MDDIFListFile.FILE_GROUP_ATTRIBUTE, description="The group that owns the file in filesystem"),
        @WritesAttribute(attribute= MDDIFListFile.FILE_SIZE_ATTRIBUTE, description="The number of bytes in the file in filesystem"),
        @WritesAttribute(attribute= MDDIFListFile.FILE_PERMISSIONS_ATTRIBUTE, description="The permissions for the file in filesystem. This " +
                "is formatted as 3 characters for the owner, 3 for the group, and 3 for other users. For example " +
                "rw-rw-r--"),
        @WritesAttribute(attribute= MDDIFListFile.FILE_LAST_MODIFY_TIME_ATTRIBUTE, description="The timestamp of when the file in filesystem was " +
                "last modified as 'yyyy-MM-dd'T'HH:mm:ssZ'"),
        @WritesAttribute(attribute= MDDIFListFile.FILE_LAST_ACCESS_TIME_ATTRIBUTE, description="The timestamp of when the file in filesystem was " +
                "last accessed as 'yyyy-MM-dd'T'HH:mm:ssZ'"),
        @WritesAttribute(attribute= MDDIFListFile.FILE_CREATION_TIME_ATTRIBUTE, description="The timestamp of when the file in filesystem was " +
...
public abstract class MDDIFListFile extends AbstractListProcessor<FileInfo> {
    ...
    protected Map<String, String> createAttributes(final FileInfo fileInfo, final ProcessContext context) {
        ...
        if (includeFileAttributes) {
            ...
            try {
                attributes.put("Hostname",InetAddress.getLocalHost().getHostName()); //Host Name is to be added.
